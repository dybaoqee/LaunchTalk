<?php
	...
	define('ECKO_THEME_VERSION', '2.2.0');
	...
	function cedar_theme_setup(){
		...
		add_theme_support('post-formats', array('status', 'aside', 'image', 'quote', 'video', 'audio', 'link'));
		register_nav_menus( 
			array(
	...
	function cedar_load_scripts(){
		if(!is_admin()){
			...
			if(get_theme_mod('general_use_extended_char_set')){
				wp_register_style('cedar_fonts', '//fonts.googleapis.com/css?family=Noto+Serif:400,700|Montserrat|Varela+Round|Oxygen&subset=latin,latin-ext', array(), 'cedar');
			}else{
				wp_register_style('cedar_fonts', '//fonts.googleapis.com/css?family=Noto+Serif:400,700|Montserrat|Varela+Round|Oxygen', array(), 'cedar');
			}
			...
			wp_localize_script('ecko_js', 'ecko_theme_vars', array(
				...
				'localization_viewcomments' => esc_js(esc_html__('View Comments', 'cedar')),
				'localization_comments' => esc_js(esc_html__('Comments', 'cedar'))
			));
		}
	}
